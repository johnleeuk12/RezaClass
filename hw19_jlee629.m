...
img_average = reshape(mu,[192,168]); %Average face
eface1 = reshape(Ud(:,1),192,168); %eigenface 1
eface2 = reshape(Ud(:,2),192,168); %eigenface 2 
...
y1=-std(Y(1,:)):0.2*std(Y(1,:)):std(Y(1,:)); % plotting faces captured by first and second principal components
Z1=mu+Ud(:,1)*y1;
